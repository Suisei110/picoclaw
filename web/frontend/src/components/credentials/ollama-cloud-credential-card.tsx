...
interface OllamaCloudCredentialCardProps {
...
export function OllamaCloudCredentialCard({
  status,
  activeAction,
  token,
  onTokenChange,
  ...
  onSaveToken,
  onAskLogout,
}: OllamaCloudCredentialCardProps) {
  const { t } = useTranslation()
  const actionBusy = activeAction !== ""
  const tokenLoading = activeAction === "ollama-cloud:token"
  ...
  return (
    <CredentialCard
      ...
      description={t("credentials.providers.ollama-cloud.description")}
          ...
          <div className="flex h-full flex-col gap-3">
            <div className="flex h-full items-center gap-2">
              <Input
                ...
                type="password"
                placeholder={t("credentials.fields.ollamaCloudToken")}
              />
              <Button
                size="sm"
                className="w-fit"
      ...
      footer={
        status?.logged_in ? (
          <Button
            ...
            className="text-destructive hover:bg-destructive/10 hover:text-destructive"
          >
            {activeAction === "ollama-cloud:logout" && (
              <IconLoader2 className="size-4 animate-spin" />
            )}
